...
STEPS = [
    Step(
        ...
        "Wskaż folder, w którym ma znaleźć się piper-mat.",
        "Najprościej zostawić domyślny folder Dokumenty. Program utworzy w nim katalog „piper-mat”. Nie wybieraj pendrive'a ani katalogu tymczasowego.",
        "Wybierz folder",
        "choose_folder",
    ),
    Step(
        2,
        "Pobierz albo zaktualizuj repozytorium",
        "Program pobierze pliki projektu z GitHub.",
        "Repozytorium to po prostu folder z kodem programu. Jeśli masz go już na komputerze, ten krok tylko pobierze najnowsze zmiany. Twoje lokalne wyniki treningu nie są przez to usuwane.",
        "Pobierz / zaktualizuj",
        "clone_or_update",
    ),
    Step(
        3,
        "Pobierz duże pliki treningowe",
        "Git LFS pobierze nagrania WAV i bazowy checkpoint.",
        "Część plików jest zbyt duża dla zwykłego Git. Git LFS przechowuje je osobno. Ten krok może potrwać i pobrać dużo danych. Bez niego trening nie ruszy.",
        "Pobierz duże pliki",
        "git_lfs_pull",
    ),
    Step(
        4,
        "Utwórz prywatne środowisko Pythona",
        "Program utworzy folder .venv tylko dla tego projektu.",
        "To osobny, bezpieczny zestaw bibliotek. Dzięki temu instalacja Pipera nie miesza się z innymi programami na komputerze. Ten krok wykonuje się zwykle tylko raz.",
        "Utwórz środowisko",
        "create_venv",
    ),
    Step(
        5,
        "Zainstaluj biblioteki do treningu",
        "Zainstalowane zostaną PyTorch, Lightning i pozostałe składniki.",
        "To właściwe narzędzia, których Piper potrzebuje do uczenia głosu. Pobieranie może potrwać kilka minut i zajmie kilka GB. Nie zamykaj okna w trakcie instalacji.",
        "Zainstaluj biblioteki",
        "install_dependencies",
    ),
    Step(
        6,
        "Zbuduj brakujący moduł treningowy",
        "Program skompiluje monotonic_align wymagany przez Piper.",
        "To techniczny moduł używany podczas dopasowywania tekstu do dźwięku. Jeśli Windows nie ma potrzebnych narzędzi C++, program pokaże czytelny komunikat i link do Visual Studio Build Tools.",
        "Zbuduj moduł",
        "build_extension",
    ),
    Step(
        7,
        "Sprawdź nagrania",
        "Program sprawdzi metadane i pliki WAV przed treningiem.",
        "Ten krok nie zmienia nagrań. Szuka brakujących plików, złego formatu, ciszy, przesterowania i innych problemów, które mogłyby zepsuć trening.",
        "Sprawdź nagrania",
        "validate_dataset",
    ),
    Step(
        8,
        "Sprawdź, czy komputer jest gotowy",
        "Uruchomiona zostanie pełna kontrola konfiguracji treningu.",
        "Program sprawdzi m.in. checkpoint, Git LFS, moduły Pythona, moduł monotonic_align, miejsce na dysku i możliwość odczytania checkpointu na Windows. Zielony wynik oznacza, że można trenować.",
        "Sprawdź gotowość",
        "check_ready",
    ),
    Step(
        9,
        "Sprawdź plan treningu",
        "Zobaczysz liczbę zaplanowanych sesji i aktualny postęp.",
        "Trening jest podzielony na kilka podejść. Po każdej sesji zapisuje się checkpoint i raport. Możesz wyłączyć komputer i wrócić do treningu następnego dnia.",
        "Pokaż plan",
        "training_status",
    ),
    Step(
        10,
        "Uruchom następną sesję treningu",
        "Piper będzie trenował do końca najbliższego zaplanowanego etapu.",
        "To długi krok. Podczas treningu komputer będzie mocno używał GPU. Po zakończeniu program zapisze postęp, najlepsze checkpointy i raport z wykresami. Dopiero wtedy bezpiecznie wyłącz komputer.",
        "START następnej sesji",
        "start_training",
    ),
    Step(
        11,
        "Otwórz raport z ostatniej sesji",
        "Po treningu możesz obejrzeć opis wyników i wykresy.",
        "Raport pokazuje przebieg metryk treningowych. Nie musisz rozumieć każdego wykresu. Najważniejsze jest, że raport pozwala porównać kolejne sesje i wykryć pogorszenie modelu.",
        "Otwórz raport",
        "open_report",
    ),
]
...
class Wizard(Tk):
    ...
    def _build_ui(self) -> None:
        ...
        for idx, step in enumerate(STEPS):
            btn = ttk.Button(
                ...
                command=lambda i=idx: self._show_step(i),
                width=30,
            )
            btn.pack(fill=X, pady=2)
            self.step_list.append(btn)
        ...
        self.path_label = ttk.Label(path_box, textvariable=self.repo_dir, wraplength=650)
        ...
        ttk.Checkbutton(
            controls,
            text="Po sukcesie przejdź automatycznie do następnego kroku",
            variable=self.auto_continue,
        ).pack(side=LEFT, padx=14)

        nav = ttk.Frame(right)
        nav.pack(fill=X, pady=(0, 10))
        self.prev_button = ttk.Button(nav, text="← Poprzedni krok", style="Nav.TButton", command=self._previous)
        self.prev_button.pack(side=LEFT)
        self.next_button = ttk.Button(nav, text="Następny krok →", style="Nav.TButton", command=self._next)
    ...
    def _run_current_step(self) -> None:
        ...
        if step.action == "choose_folder":
            action()
            return
        if step.action == "start_training":
            if not messagebox.askyesno(
                "Uruchomić trening?",
                "Trening może trwać wiele godzin i mocno obciąży kartę graficzną.\n\n"
    ...
    def _action_choose_folder(self):
        ...
        if selected:
            ...
            candidate = parent if parent.name.lower() == REPO_NAME else parent / REPO_NAME
            self.repo_dir.set(str(candidate))
    ...
    def _require_repo(self) -> tuple[bool, str]:
        ...
        if not (repo / ".git").is_dir():
            return False, "Najpierw wykonaj krok pobierania repozytorium."
        return True, ""
    ...
    def _action_git_lfs_pull(self):
        ...
        if not ok:
            return ok, msg
        repo = self._repo_path()
        rc = self._run_command(["git", "lfs", "install"], cwd=repo)
        if rc != 0:
    ...
    def _action_create_venv(self):
        ...
        rc = self._run_command([python, "-m", "venv", ".venv"], cwd=repo)
        if rc != 0:
            return False, "Nie udało się utworzyć środowiska .venv."
        if not self._venv_python().is_file():
            return False, "Folder .venv powstał, ale nie znaleziono jego python.exe."
        return True, "Prywatne środowisko Pythona jest gotowe."
    ...
    def _action_install_dependencies(self):
        ...
        commands = [
            ...
            [str(py), "-m", "pip", "install", "-e", ".[train]"],
        ]
        for command in commands:
            if self._run_command(command, cwd=repo) != 0:
    ...
    def _action_build_extension(self):
        ...
        repo = self._repo_path()
        rc = self._run_command([str(py), "setup.py", "build_ext", "--inplace"], cwd=repo)
        if rc != 0:
            self.log_queue.put("Jeśli komunikat mówi o brakującym kompilatorze C++, zainstaluj Visual Studio 2022 Build Tools z pakietem 'Desktop development with C++'.")
            return False, "Nie udało się zbudować modułu Cython. Najczęściej brakuje Visual Studio Build Tools z obsługą C++."
        return True, "Moduł treningowy został zbudowany."
    ...
    def _action_validate_dataset(self):
        ...
        rc = self._run_command(
            [str(py), "scripts/validate_dataset.py", "--metadata", "dataset/metadata.csv", "--audio-dir", "dataset/wavs"],
            cwd=repo,
        )
    ...
    def _action_start_training(self):
        ...
        repo = self._repo_path()
        ready = self._run_command([str(py), "scripts/check_training_ready.py"], cwd=repo)
        if ready != 0:
    ...
    def _action_open_report(self):
        ...
        self.log_queue.put(f"Wykresy: {folder}")
